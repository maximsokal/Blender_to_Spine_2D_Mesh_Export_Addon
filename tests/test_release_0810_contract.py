...
SAMPLE_GENERATOR = (
    ...
    / "tests"
    / "blender_headless"
    / "generate_depth_camera_projection_samples.py"
)
...
def test_release_note_records_depth_surface_and_base_policy() -> None:
    ...
    assert "all other retained points receive non negative offsets toward the camera" in normalized
    assert "no generated point is allowed to extend away from the base" in normalized
    assert "object origin is implemented as a hidden fail closed policy" in normalized
    assert "0.81.0 ui exposes and uses only farthest visible point" in normalized
...
def test_release_note_records_target_and_sequence_acceptance_matrix() -> None:
    note = _read(RELEASE_NOTE)
    normalized = _normalized(note)
    ...
    assert "two frame spine 4.2 depth camera projection sequence" in normalized
