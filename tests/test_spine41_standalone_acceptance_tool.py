"""Pure-Python contracts for the Blender/runtime Spine 4.1 acceptance gate."""
...
from tools import run_spine41_standalone_acceptance as acceptance
...
def test_oracle_command_passes_external_runtime_only_as_input(tmp_path: Path) -> None:
    ...
    runtime = tmp_path / "vendor" / "spine-webgl-41" / "index.js"
...
def test_prepare_output_root_rejects_non_empty_directory_without_replace(
    ...
    with pytest.raises(
        acceptance.Spine41StandaloneAcceptanceError,
        match="not empty",
    ):
