...
def test_material_and_production_callers_use_physical_analysis_owner():
    material_source = _source("material_analyzer.py")
    production_source = _source("production_shader_capabilities.py")
    package_source = _source("__init__.py")

    assert "from .shader_graph_analysis import" in material_source
    assert "from .shader_graph_rna import" in material_source
