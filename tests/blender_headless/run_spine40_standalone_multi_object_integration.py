"""Generate and validate a real Blender 5.2 Spine 4.0 standalone export."""
...
PREFIXES = ("Spine40A", "Spine40B", "Spine40C")
...
def _build_object_settings(
    ...
    *,
    ...
    """Build exact production settings for one independent Spine 4.0 object rig."""
    ...
    return A1SingleObjectExportSettings(
        export=ExportSettings(
            ...
            spine_version=SpineJsonTarget.SPINE_4_0.exact_version,
...
def _build_sources(
    ...
    for index, (prefix, component_id, position) in enumerate(
        ...
        source_object = _create_quad(f"Spine40Source{index}")
...
def _all_constraints(document: dict[str, object]) -> tuple[dict[str, object], ...]:
    """Return all ordered legacy constraint collections."""
...
def _assert_same_owner_reference(
    ...
    *,
    ...
    """Reject cross-object generated bone references."""
    ...
    for raw_name in names:
        ...
        if raw_name == "root":
            continue
        _assert(
            _owner_prefix(raw_name) == owner,
            f"{label} crosses object rigs: owner={owner}, reference={raw_name!r}",
        )


def _walk_json(value: object, *, path: str = "document") -> Iterable[tuple[str, object]]:
    """Yield every JSON member path and value for target-field audits."""
...
def _assert_standalone_document(document: dict[str, object]) -> dict[str, object]:
    """Validate exact 4.0 schema and independent per-object ownership."""

    skeleton = document.get("skeleton")
    _assert(isinstance(skeleton, dict), "skeleton metadata is missing")
    _assert(
        skeleton.get("spine") == SpineJsonTarget.SPINE_4_0.exact_version,
        f"unexpected Spine version: {skeleton.get('spine')!r}",
    )
    _assert("referenceScale" not in skeleton, "referenceScale leaked into Spine 4.0")
    _assert("physics" not in document, "physics leaked into Spine 4.0")

    for path, _value in _walk_json(document):
        _assert(not path.endswith(".sequence"), f"sequence leaked into Spine 4.0: {path}")
    ...
    path_constraints = _json_array(document, "path")

    _assert(bones, "bones must be non-empty")
    _assert(len(slots) == 3, f"expected 3 slots, got {len(slots)}")
    _assert(len(skins) == 1, "exactly one skin is required")
    _assert(len(ik) == 3, f"expected 3 IK constraints, got {len(ik)}")
    _assert(
        len(transform) == 12,
        f"expected 12 transform constraints, got {len(transform)}",
    )
    _assert(not path_constraints, "standalone acceptance expects no path constraints")

    bone_by_name: dict[str, dict[str, object]] = {}
    for item in bones:
        _assert(isinstance(item, dict), "bones contains a non-object")
        name = item.get("name")
        ...
        _assert("inherit" not in item, f"4.2 inherit field leaked: {name}")
        transform_mode = item.get("transform")
        _assert(
            transform_mode is None or isinstance(transform_mode, str),
            f"bone {name} has invalid transform mode",
        )
        bone_by_name[name] = item
    ...
    for name, bone in bone_by_name.items():
        ...
        _assert(owner is not None, f"unowned standalone bone: {name}")
    ...
    skin = skins[0]
    _assert(isinstance(skin, dict), "skin is not an object")
    _assert("constraints" not in skin, "4.2 skin.constraints leaked")
    _assert(isinstance(skin.get("attachments"), dict), "skin attachments are missing")
    for field_name, collection in (("ik", ik), ("transform", transform)):
        ...
        _assert(
            all(isinstance(name, str) and name in names for name in membership),
...
def _assert_state_restored(
    *,
    ...
    _assert(_capture_context() == context_before, "Spine 4.0 export changed context")
    _assert(
        _capture_scene_bake_state() == scene_before,
        "Spine 4.0 export changed scene bake state",
    )
    _assert(
        tuple(_material_fingerprint(material) for material in materials)
        == material_fingerprints,
        "Spine 4.0 export mutated source materials",
    )
    _assert(
        not _temporary_datablock_names(),
        "Spine 4.0 export leaked temporary Blender datablocks",
    )


def run(output_directory: Path) -> Path:
    """Run the production export and return the generated JSON path."""
    ...
    _assert(result.success, f"Spine 4.0 standalone export failed: {result.issues}")
...
def main() -> None:
    ...
    print("[SPINE40_STANDALONE] RUN production multi-object export")
    json_path = run(arguments.output)
    print(f"[SPINE40_STANDALONE] JSON {json_path}")
    print("[SPINE40_STANDALONE] PASS production multi-object export")
