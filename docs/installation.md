...
5. Select `blender_to_spine2d_mesh_exporter-0.55.5.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.55.5.zip` through **Install from Disk**.
5. Reopen the project file.

Closing Blender prevents loaded Python modules and cached extension metadata from keeping the previous implementation active. Always export a fresh JSON after installing 0.55.5; do not reuse JSON generated by earlier development candidates when validating the signed-axis projection work or Spine 4.1 scale behavior.

## Scene settings migration

Version 0.55.5 continues to use Scene settings schema 6, introduced in 0.55.1. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
The 0.55.5 candidate preserves the Scene schema while retaining the limited Spine 4.1.24 serializer path, the scale-preserving target-aware 2-Axis document topology, and the internal signed-axis projection foundation. No additional `.blend` migration is required in this slice because the public projection dropdown is not registered yet and existing requests default to `+Z`.
...
```text
 dist/blender_to_spine2d_mesh_exporter-0.55.5.zip
```
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.55.5.zip
```
