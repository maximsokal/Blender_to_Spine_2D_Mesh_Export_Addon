...
Spine 4.1 connected, mixed, and 3-Axis export remain blocked by production capability checks. Production **Export Selected Objects** always builds standalone object rigs. Hidden Connect values retained by older `.blend` files are ignored; connected and mixed composition remain development-only internal capabilities.
...
5. Select `blender_to_spine2d_mesh_exporter-0.47.11.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.47.11.zip` through **Install from Disk**.
5. Reopen the project file.

Closing Blender prevents loaded Python modules and cached extension metadata from keeping the previous implementation active. Always export a fresh JSON after installing 0.47.11. Version 0.47.11 specifically prevents persisted hidden Connect flags from routing an ordinary selected-object export into the development-only connected pipeline.

## Scene settings migration

Version 0.47.11 owns Scene settings schema 5. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
The 0.47.11 patch does not change the Scene schema. It keeps the limited Spine 4.1.24 scale-preserving topology from 0.47.10 and separates production standalone routing from the persisted development-only Connect property, so no additional `.blend` migration is required.
...
```text
 dist/blender_to_spine2d_mesh_exporter-0.47.11.zip
```
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.47.11.zip
```
...
Also reopen a `.blend` that previously used Connect settings and verify that **Export Selected Objects** produces a standalone JSON without `all_objects_*` bones.
...
Existing custom properties stored in a `.blend` may remain serialized until the file is saved without them; they do not execute code after the extension is removed and no longer affect production selected-object routing.
