...
5. Select `blender_to_spine2d_mesh_exporter-0.47.10.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.47.10.zip` through **Install from Disk**.
5. Reopen the project file.

Closing Blender prevents loaded Python modules and cached extension metadata from keeping the previous implementation active. Always export a fresh JSON after installing 0.47.10; do not reuse JSON generated by earlier development candidates when validating Spine 4.1 scale behavior.

## Scene settings migration

Version 0.47.10 owns Scene settings schema 5. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
The 0.47.10 patch does not change the Scene schema. It adds the limited Spine 4.1.24 serializer path and a scale-preserving target-aware 2-Axis document topology, so no additional `.blend` migration is required.
...
```text
 dist/blender_to_spine2d_mesh_exporter-0.47.10.zip
```
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.47.10.zip
```
