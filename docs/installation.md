...
5. Select `blender_to_spine2d_mesh_exporter-0.55.17.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.55.17.zip` through **Install from Disk**.
5. Reopen the project file.

Closing Blender prevents loaded Python modules and cached extension metadata from keeping the previous implementation active. Always export a fresh JSON after installing 0.55.17; do not reuse JSON generated by earlier development candidates when validating signed-axis projection, Active Camera Normal - UV Segments placement, standalone/connected/mixed object-block draw order, or Spine 4.1 scale behavior.

## Scene settings migration

Version 0.55.17 continues to use Scene settings schema 6, introduced in 0.55.1. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
The 0.55.17 candidate preserves the Scene schema while retaining the limited Spine 4.1.24 serializer path and scale-preserving target-aware 2-Axis topology. Typed Normal - UV Segments requests can project evaluated geometry through the active Perspective or Orthographic camera using export texture dimensions. Connected internal composition places the global group main at the selected anchor's absolute projected Object Origin, keeps object mains anchor-relative, groups hierarchy layers by projected origin depth, and orders complete setup slot blocks by nearest evaluated vertex depth. Mixed internal composition applies the same object-block order across connected and standalone subgroup boundaries. The 0.55.16 affine setup evaluator remains unchanged. Candidate 0.55.17 corrects only its architecture contract so the Active Camera worker is recognized as the setup-oracle owner and the signed-axis worker as the shared consumer. Production Slice 5 composition is unchanged. Existing rendered Camera Projection and public standalone-only UI routing remain unchanged. Spine 4.1 connected and mixed capability checks remain fail-closed. No additional `.blend` migration is required because the public projection dropdown is not registered yet and existing requests default to `+Z`.
...
```text
 dist/blender_to_spine2d_mesh_exporter-0.55.17.zip
```
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.55.17.zip
```
