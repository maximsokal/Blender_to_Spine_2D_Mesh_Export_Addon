...
5. Select `blender_to_spine2d_mesh_exporter-0.47.5.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.47.5.zip` through **Install from Disk**.
5. Reopen the project file.

Closing Blender prevents loaded Python modules and cached extension metadata from keeping the previous implementation active. For connected-rig validation, always export a new JSON after installing 0.47.5; do not reuse JSON produced by 0.47.4 or older.

## Scene settings migration

Version 0.47.5 owns Scene settings schema 5. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
The 0.47.5 patch does not change the Scene schema. It replaces the connected-document rig assembly and constraint schedule only, so no additional `.blend` migration is required.
...
```text
 dist/blender_to_spine2d_mesh_exporter-0.47.5.zip
```
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.47.5.zip
```
