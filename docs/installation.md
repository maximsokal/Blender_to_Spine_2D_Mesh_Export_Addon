...
5. Select `blender_to_spine2d_mesh_exporter-0.55.12.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.55.12.zip` through **Install from Disk**.
5. Reopen the project file.

Closing Blender prevents loaded Python modules and cached extension metadata from keeping the previous implementation active. Always export a fresh JSON after installing 0.55.12; do not reuse JSON generated by earlier development candidates when validating signed-axis projection, Active Camera Normal - UV Segments placement, standalone object-block draw order, or Spine 4.1 scale behavior.

## Scene settings migration

Version 0.55.12 continues to use Scene settings schema 6, introduced in 0.55.1. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
The 0.55.12 candidate preserves the Scene schema while retaining the limited Spine 4.1.24 serializer path and scale-preserving target-aware 2-Axis topology. Typed Normal - UV Segments requests can project evaluated geometry through the active Perspective or Orthographic camera using export texture dimensions. Active Camera now completes segmentation, decomposition and strict triangulation in normalized world geometry before projecting immutable triangle regions, preventing Perspective n-gons from failing only because retained U/V/depth coordinates are nonlinear. Standalone composition still orders complete object slot blocks from far to near by nearest camera-space vertex. Connected and mixed Active Camera requests remain blocked until their dedicated normalization slice. No additional `.blend` migration is required because the public projection dropdown is not registered yet and existing requests default to `+Z`.
...
```text
 dist/blender_to_spine2d_mesh_exporter-0.55.12.zip
```
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.55.12.zip
```
