...
5. Select `blender_to_spine2d_mesh_exporter-0.47.1.zip`.
...
4. Install `blender_to_spine2d_mesh_exporter-0.47.1.zip` through **Install from Disk**.
...
Version 0.47.1 owns Scene settings schema 5. Raw persisted Scene ID-properties are captured before Rewrite RNA properties are registered, so Blender's newly bound defaults cannot hide the values stored in an older `.blend` file.
...
```text
blender --command extension validate dist/blender_to_spine2d_mesh_exporter-0.47.1.zip
```
