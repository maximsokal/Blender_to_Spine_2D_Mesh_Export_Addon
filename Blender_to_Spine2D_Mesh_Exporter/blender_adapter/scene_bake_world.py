...
def input_socket(node: Any, socket_name: str) -> Any | None:
    ...
    try:
        return next(
            ...
            if str(getattr(socket, "name", "")) == socket_name
        )
    except Exception:
        return None
...
def background_strength(world: Any) -> float | None:
    ...
    try:
        return max(
            0.0,
            finite_float(
                getattr(strength, "default_value", 0.0),
                label="World Background Strength",
            ),
        )
...
def analyse_world(scene: Any) -> WorldBakeSnapshot | None:
    ...
    node_tree = getattr(world, "node_tree", None)
    has_node_tree = node_tree is not None
    try:
        node_types = tuple(
            sorted(
                {
    ...
    try:
        return WorldBakeSnapshot(
            ...
            color=color_tuple(
                ...
                default=(0.0, 0.0, 0.0),
                label=f"World '{world_name}' color",
            ),
            use_nodes=has_node_tree,
        ...
        raise SceneBakeAnalysisError(
            f"Unable to build World snapshot for '{world_name}': {exc}"
        ) from exc
