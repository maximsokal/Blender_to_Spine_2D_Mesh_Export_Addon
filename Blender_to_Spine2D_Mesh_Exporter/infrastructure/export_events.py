...
class ExportEvent:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.kind, ExportEventKind):
            raise TypeError("kind must be ExportEventKind")
    ...
    def emit(self, event: ExportEvent) -> None:
        ...
        for listener in listeners:
            try:
                ...
                logger.exception(
                    "Export event listener failed for %s",
                    event.kind.value,
                )

        log_method = (
            logger.error
            if event.kind is ExportEventKind.CLEANUP_FAILED
            else logger.debug
        )
        log_method(
            "[%s] %s%s",
            event.operation_id,
            event.message,
            "" if event.path is None else f" path={event.path}",
        )
