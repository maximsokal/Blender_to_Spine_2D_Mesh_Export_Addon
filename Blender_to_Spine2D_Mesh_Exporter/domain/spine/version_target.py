...
This module is Blender-independent. UI code stores the enum identifier, application
settings store the exact version string, and serializers resolve both through this one
registry instead of scattering version string comparisons across the pipeline.
"""
...
class SpineJsonVersionDescriptor:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.exact_version.startswith(f"{self.family}."):
            raise ValueError(
                "exact_version must belong to family "
                f"{self.family!r}, got {self.exact_version!r}"
            )
        for field_name in (
...
class SpineJsonTarget(str, Enum):
    """Stable RNA/application identifiers for every supported JSON target."""
...
_DESCRIPTORS: Mapping[SpineJsonTarget, SpineJsonVersionDescriptor] = MappingProxyType(
    {
        SpineJsonTarget.SPINE_3_8: SpineJsonVersionDescriptor(
            ...
            label="Spine 3.8",
            description="Export setup-pose JSON for Spine 3.8.99",
        ...
        SpineJsonTarget.SPINE_4_0: SpineJsonVersionDescriptor(
            ...
            label="Spine 4.0",
            description="Export setup-pose JSON for Spine 4.0.64",
            uses_legacy_bone_transform_field=True,
...
_TARGET_BY_EXACT_VERSION: Mapping[str, SpineJsonTarget] = MappingProxyType(
    {target.exact_version: target for target in SpineJsonTarget}
)
_TARGET_BY_FAMILY: Mapping[str, SpineJsonTarget] = MappingProxyType(
...
def spine_json_target_enum_items() -> Tuple[Tuple[str, str, str], ...]:
    """Return deterministic Blender ``EnumProperty`` items without importing bpy."""
...
def resolve_spine_json_target(value: object) -> SpineJsonTarget:
    """Resolve an enum, RNA identifier, exact version, or family token.

    This permissive resolver is intended for UI and migration boundaries. The
    application contract uses :func:`resolve_spine_json_exact_version` so a family-only
    string can never leak into ``skeleton.spine``.
    """
    ...
    exact_match = _TARGET_BY_EXACT_VERSION.get(normalized)
    if exact_match is not None:
        return exact_match
    family_match = _TARGET_BY_FAMILY.get(normalized)
...
def resolve_spine_json_exact_version(value: object) -> SpineJsonTarget:
    """Resolve only a registered exact version string for application settings."""
