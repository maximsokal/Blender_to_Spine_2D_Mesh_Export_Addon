...
def _edge_pairs(
    ...
    """Return boundary edges first, then deterministic internal diagonals."""

    if vertex_count < 3:
        raise A1DocumentAssemblyError(
            "camera projection contour requires at least 3 vertices"
        )
    if len(triangle_indices) != vertex_count - 2:
        raise A1DocumentAssemblyError(
            "camera projection triangle count does not match contour vertex count"
        )
    ...
    if len(resolved) != expected_edge_count:
        raise A1DocumentAssemblyError(
            "camera projection triangulation produced an unexpected edge count; "
...
def build_camera_projection_mesh_snapshot(
    ...
    *,
    ...
    """Create a triangulated screen-space contour in the current rig units."""
    ...
    points = resolved_layout.contour
    ...
    snapshot = MeshSnapshot(
        snapshot_id=f"{plan.source_object_id}:camera-projection-contour",
        source_object_id=plan.source_object_id,
        object_name=f"{plan.source_object_id}_CameraProjectionContour",
        vertices=vertices,
        edges=edges,
        loops=tuple(loops),
        faces=tuple(faces),
