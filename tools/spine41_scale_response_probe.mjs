...
/**
 * Validate generated two-axis scale controls with the exact read-only Spine 4.1 runtime.
 *
 * Usage:
 *   node tools/spine41_scale_response_probe.mjs <json-file> <runtime-entry>
 *
...
function bindAtlasPageTextures(atlas) {
  if (!isRecord(atlas) || !Array.isArray(atlas.pages) || atlas.pages.length === 0) {
    fail('Spine 4.1 TextureAtlas must expose at least one page');
  }
...
function collectScaleControlSpecs(document) {
  ...
  for (let index = 0; index < transforms.length; index += 1) {
    ...
    if (!constrainedBones.includes(driver) || constrainedBones.includes(unsafeDriver)) {
      fail(`Scale constraint '${name}' uses the wrong Spine 4.1 driver`, {
...
async function main() {
  ...
  const runtimeEntry = resolveInputFile(process.argv[3], 'Spine 4.1 runtime entry');
  ...
  if (!version.startsWith('4.1')) {
    fail(`Expected Spine 4.1 JSON, received ${version}`);
  }
